  ...
  - dimension: user_type
    sql: 
      CASE WHEN ${TABLE}.is_elation_staff = 0 AND ${TABLE}.time_credentialed IS NOT NULL AND ${entities_practice.app_type} = 'emr' AND ${entities_practice.emr_type} = 'practicing' 
           ...
           WHEN ${practicians_officestaff.id} AND ${TABLE}.is_elation_staff = 0 AND ${entities_practice.app_type} = 'emr' AND ${entities_practice.emr_type} = 'practicing' IS NOT NULL THEN 'staff'
        ELSE NULL
      END
